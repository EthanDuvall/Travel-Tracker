...
import { getUserId, getUsersTrips, getDestination,getTripExpenses,filterTrips} from "./user";
...
submitButton.addEventListener("click",()=>{
  let desnation = getCheckedDesntionation()
  let newTrip ={
    id:Date.now,
    userId:userData.id,
...
function getCheckedDesntionation(){
  return desnationInput.find(destination => {
...
let total = 0
  destinations.forEach(desntination =>{
    total = Math.round(((desntination.estimatedLodgingCostPerDay *tripVaule.duration) + (desntination.estimatedFlightCostPerPerson *  tripVaule.traveler))*.1)
    formDisplayArea.insertAdjacentHTML("beforeend",`
    <div>
      <label for="destination">${desntination.destination}</label>
      <input type="radio" name="destination" id="${desntination.id}" required/>
      <aside>
        <p>Lodging Cost : ${desntination.estimatedLodgingCostPerDay *tripVaule.duration } $</p>
        <p>Flight Cost : ${desntination.estimatedFlightCostPerPerson *  tripVaule.traveler}$</p>
